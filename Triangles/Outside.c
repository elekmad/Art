...
{
    int animated = 1, nb_triangles_max = 30, geometric = 0, distance = -15;
    SDL_Window *window;
    ...
    double center_x = SCREEN_WIDTH / 2, center_y = SCREEN_HEIGTH / 2, distance_from_center = (SCREEN_WIDTH < SCREEN_HEIGTH ? SCREEN_WIDTH : SCREEN_HEIGTH) * 0.2, size = 50;//(SCREEN_WIDTH < SCREEN_HEIGTH ? SCREEN_WIDTH : SCREEN_HEIGTH) * 0.5;
    for(int num_triangle = 0; num_triangle < 6; num_triangle++)
    {
        double triangle[6] = {center_x, center_y,
				center_x + size * cos(num_triangle * M_PI / 3), center_y + size * sin(num_triangle * M_PI / 3),
				center_x + size * cos((num_triangle + 1) * M_PI / 3), center_y + size * sin((num_triangle + 1) * M_PI / 3)};
	for(int cmpt = 0; cmpt < 3; cmpt++)
	{
